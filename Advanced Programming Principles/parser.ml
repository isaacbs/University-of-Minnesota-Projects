...
module Parser: Parsers = struct
  exception Can'tParse of string;;

  let makeParser path = 
      let nToken = Scanner.makeScanner path
      in let token = ref Scanner.EndToken
        in

  let rec nextThings () =
      match !token with 
      Scanner.CloseParenToken -> token:= nToken () ;
      Nil |
      Scanner.EndToken ->
      raise (Can'tParse "Expression ended without a CloseParenToken") |
      _ -> 
      let temp = nextThing () in 
      let temp2 = nextThings () in
      Cons(temp, temp2)
  ...
  and nextThing () = 
    ...
    Scanner.CloseParenToken ->  
      raise (Can'tParse "Can not parse CloseParenToken")|
    Scanner.EndToken -> 
      raise (Can'tParse "Unexpected end of file")|
    Scanner.NumberToken(n) -> token := nToken () ;
      Number(n)|
